  ...
  return (
    props.modalToggle && (
      <div className="fixed inset-0 flex justify-center items-center bg-opacity-50 z-50 w-1/2 m-auto">
        <div className="relative border-gray-400 bg-slate-50 rounded-md text-blue-200 h-3/4 w-3/4 p-4 shadow-lg">
          {/* Close Button */}
          <button
          ...
          <div className="flex justify-center items-center text-black container">
            ...
            <input
              type="text"
              className="bg-opacity-10 opacity-50 text-center border-b-2 outline-none text-black inline w-8 mx-2"
              value={value}
              onChange={(v) => {
          ...
          <div className="flex justify-center items-center py-2">
            <button
              className="container w-24 bg-[#00205B] hover:bg-orange-500 cursor-pointer mt-5 transition duration-300 px-4 py-2 rounded-md text-orange-500 hover:text-[#00205B]"
              onClick={() => {
                ...
                Object.keys(newTrackedMeals).forEach((meal) => {
                  newTrackedMeals[meal].forEach((macro) => {
                    if (
                      !Object.keys(newTrackedNutrition).includes(macro.name)
                    ) {
                ...
                localStorage.setItem(
                  "trackedMeals",
                  JSON.stringify(newTrackedMeals)
                );
                localStorage.setItem(
                  "trackedNutrition",
                  JSON.stringify(newTrackedNutrition)
                );
